...
class Servidor:
    ...
    def client (self, conexao, docliente):
        ...
        View.inserir_cliente(docliente[0])
        ...
        while True: # Loop para receber mensagnes do cliente
            ...
            try:
                ...
                cliente = View.buscar_cliente_ip(docliente[0])
                c = View.inserir_criptografado(cliente.id, str(enc_text))
                partes = dec_text.split(", ")
                ...
                for parte in partes:
                    if "cpu:" in parte:
                        cpu = float(parte.split(": ")[1])
                    elif "mem:" in parte:
                        mem = float(parte.split(": ")[1])
                    elif "disk:" in parte:
                        disk = float(parte.split(": ")[1])
                    elif "temp:" in parte:
                        temp = float(parte.split(": ")[1])

                View.inserir_descriptografado(cliente.id, c.id, cpu, mem, disk, temp)
